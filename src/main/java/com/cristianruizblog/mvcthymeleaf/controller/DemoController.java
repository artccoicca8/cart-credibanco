...
import com.cristianruizblog.mvcthymeleaf.model.CartEntity;
import com.cristianruizblog.mvcthymeleaf.model.CartEntityCypher;
...
public class DemoController {
	...
	public String index(Model modelo) {
		...
		entity.setAcquirerId("1"); 
		entity.setCommerceId("1809");
		entity.setPurchaseTerminalCode("2907891");
		entity.setPurchaseAmount("10000");
		entity.setPurchaseOperationNumber(new Long(System.currentTimeMillis()).toString().substring(4));
		modelo.addAttribute("entity", entity);
		return "index";
	} 

	@PostMapping(path = "/stepTwo")
	public String createOrUpdateEmployee(Model modelo,CartEntity entity) {
...
//		ht.put("SESSIONKEY", vPOSBean.getCipheredSessionKey());
		...
		CartEntityCypher cypher = new CartEntityCypher();

		
		cypher.setIDACQUIRER("1");
		cypher.setIDCOMMERCE(entity.getCommerceId());
		cypher.setSESSIONKEY(vPOSBean.getCipheredSessionKey());
		cypher.setTERMINALCODE(entity.getPurchaseTerminalCode());
		cypher.setDIGITALSIGN(vPOSBean.getCipheredSignature());
		cypher.setXMLREQ(vPOSBean.getCipheredXML());

		
		modelo.addAttribute("cypher", cypher);
